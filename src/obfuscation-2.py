...
class tool:
    ...
    def __init__(self, options):   
        ...
        if self.j < 2: sys.exit("Error: J < 2")
    ...
    def records2string(self, r, bytes, packets, flags, tos, proto):
        ...
        outstr = outstr+bytes+" "+packets+" "+str(r['duration'])
        return outstr

    """ write a set of records into files """
    def writeout(self, rs):
        # IPOTESI: fare i thread qui .. 1 produttore N consumatori
        minbyte = minpacket = 999999999999999 
        maxbyte = maxpacket = -1
        ...
        for r in rs:
            if r['byte'] < minbyte: minbyte = r['byte']
            if r['byte'] > maxbyte: maxbyte = r['byte']

            if r['packet'] < minpacket: minpacket = r['packet']
            if r['packet'] > maxpacket: maxpacket = r['packet']

            flags   |= Set([ r['flag'] ])
            tos     |= Set([ r['tos'] ])
            proto   |= Set([ r['proto'] ])

        bytes   = "["+str( minbyte )+"-"+str( maxbyte )+"]"
        packets = "["+str( minpacket )+"-"+str( maxpacket )+"]"
